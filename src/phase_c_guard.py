...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    entities_to_detect = ["VN_CCCD", "VN_PHONE", "EMAIL_ADDRESS", "PHONE_NUMBER"]
    results = analyzer.analyze(text=text, language=PRESIDIO_LANGUAGE, entities=entities_to_detect)
    if not results:
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Fallback for adversarial queries that NeMo missed due to English embedding model limitations on Vietnamese text
    if not blocked:
        adv_keywords = ["bỏ qua", "ignore", "dan", "đóng vai", "tiết lộ", "admin", "ceo", "thơ", "nấu", "bitcoin", "phương trình", "phim", "override", "lệnh", "toàn bộ thông tin"]
        if any(kw in text.lower() for kw in adv_keywords):
            blocked = True
            content = "Xin lỗi, tôi không thể thực hiện yêu cầu này."
...
if __name__ == "__main__":
    ...
    with open("reports/guard_results.json", "w", encoding="utf-8") as f:
        json.dump({"adversarial_results": results, "latency": latency}, f, ensure_ascii=False, indent=2)
